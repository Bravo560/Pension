 ...
 *   4. Se agrega la Pensión Mínima Garantizada (Art. 33/58 LSS 1973 y jurisprudencia relacionada):
 *      NINGUNA pensión de vejez o cesantía en edad avanzada otorgada bajo la Ley 1973 puede ser
 *      inferior a un monto fijo mensual (equivalente al salario mínimo general × 1.11, "Factor
 *      Fox"), sin importar qué tan bajo dé el cálculo individual. El Excel original NO aplicaba
 *      este piso — es un hallazgo posterior a la auditoría original, detectado al comparar un
 *      resultado real de la app contra el monto público de la pensión mínima 2026. Se aplica
 *      DESPUÉS del tope del Art. 168 (el piso no está limitado por el 100% del salario propio:
 *      es una garantía solidaria, no depende de lo que la persona cotizó).
 *
...
// --- Pensión Mínima Garantizada (Ley 1973) ---
// Monto mensual mínimo que el IMSS debe pagar a CUALQUIER pensión de vejez o cesantía en edad
// avanzada otorgada bajo la Ley 1973, sin importar qué tan bajo dé el cálculo individual.
// Se actualiza cada 1° de enero junto con el salario mínimo general (fórmula pública: salario
// mínimo general mensual × 1.11). Vigente desde el 01-ene-2026: $10,636.53/mes.
// ACTUALIZAR CADA ENERO — puede sobreescribirse por variable de entorno
// PENSION_MINIMA_GARANTIZADA_MENSUAL o por parámetro `pensionMinimaGarantizadaMensual`.
const PENSION_MINIMA_GARANTIZADA_MENSUAL_DEFAULT = 10636.53;
...
function calcularPension(input) {
  const {
    ...
    ramo,
    edad,
    pensionMinimaGarantizadaMensual = PENSION_MINIMA_GARANTIZADA_MENSUAL_DEFAULT,
  } = input;
  ...
  // --- Pensión Mínima Garantizada (Ley 1973): piso mensual que aplica DESPUÉS del tope del
  // Art. 168. Es una garantía solidaria (no depende de cuánto cotizó la persona), así que no
  // está limitada por el 100% de su propio salario como el tope de arriba.
  const pensionMensualAntesDeMinima = pensionAnualFinalConTope / 12;
  const aplicaMinimaGarantizada = pensionMensualAntesDeMinima < pensionMinimaGarantizadaMensual;
  const pensionMensualFinalConTopeYMinima = aplicaMinimaGarantizada
    ? pensionMinimaGarantizadaMensual
    : pensionMensualAntesDeMinima;
  ...
  return {
    ...
    pensionAnualFinal, // el resultado aplicable a este ramo (vejez o cesantía), SIN aplicar el tope ni la mínima
    tope: { salarioAnualizado, pctDelSalario, excedeTope, pensionAnualFinalConTope },
    minimaGarantizada: {
      montoMensual: pensionMinimaGarantizadaMensual,
      aplicada: aplicaMinimaGarantizada,
    },
    mensual: {
      ...
      // Antes solo aplicaba el tope (Art. 168); ahora también aplica el piso legal de la
      // Pensión Mínima Garantizada. Se mantiene el mismo nombre de campo para no romper al
      // frontend, que ya lee este valor como "la pensión mensual final".
      pensionFinalConTope: pensionMensualFinalConTopeYMinima,
    },
  };
}
...
const PensionCalc = {
  ...
  PENSION_MINIMA_GARANTIZADA_MENSUAL_DEFAULT,
};
