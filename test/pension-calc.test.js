...
// Este caso ya supera la Pensión Mínima Garantizada 2026 ($10,636.53/mes), así que el piso NO
// debe alterar el resultado final (regresión: la corrección del piso no debe tocar casos que
// ya estaban por encima del mínimo).
assert.strictEqual(r.minimaGarantizada.aplicada, false);
aprox(r.mensual.pensionFinalConTope, 19048.690459143752);

// --- Pensión Mínima Garantizada (Ley 1973) ---
// Caso con salario bajo y pocas semanas (justo el mínimo de 500): el cálculo individual debe
// dar un monto menor al piso legal, y el motor debe ajustarlo al mínimo garantizado — antes de
// esta corrección, la app devolvía el monto crudo (más bajo) sin aplicar el piso.
const rBajo = calcularPension({
  semanasCotizadas: 500,
  salarioDiarioPromedio: 200,
  umaVigente: 117.31,
  tieneEsposa: false,
  numHijos: 0,
  numPadres: 0,
...
assert.ok(
  rBajo.mensual.pensionFinal < 10636.53,
  'Este caso debe dar, sin piso, un monto menor a la mínima garantizada 2026 (para que la prueba sea significativa).'
);
assert.strictEqual(rBajo.minimaGarantizada.aplicada, true);
aprox(rBajo.mensual.pensionFinalConTope, 10636.53);
aprox(rBajo.minimaGarantizada.montoMensual, 10636.53);

// El piso debe poder sobreescribirse por parámetro (para simular el monto de otro año).
const rBajoOtroAnio = calcularPension({
  semanasCotizadas: 500,
  salarioDiarioPromedio: 200,
  umaVigente: 117.31,
  ramo: 'vejez',
  pensionMinimaGarantizadaMensual: 9500,
});
aprox(rBajoOtroAnio.mensual.pensionFinalConTope, 9500);
