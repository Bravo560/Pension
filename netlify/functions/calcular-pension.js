...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Pensión Mínima Garantizada (piso legal Ley 1973): parámetro del negocio, no algo que el
    // usuario capture — se toma del servidor salvo que el body lo traiga explícito (ej. para
    // simular el monto de otro año).
    const pensionMinimaGarantizadaMensual = input.pensionMinimaGarantizadaMensual
      || Number(process.env.PENSION_MINIMA_GARANTIZADA_MENSUAL || '10636.53');
    const resultado = calcularPension({ ...input, umaVigente, pensionMinimaGarantizadaMensual });
